...
public class PCA9685_test {
     ...
     */
    /* package */ PCA9685_test(I2cDevice device) {
        Log.i("PCA","Create 2");
        this.device = device;
    }
     ...
     * @return new Pca9685
     */
    public static PCA9685_test create() {
        Log.i("PCA","Create 0");
     ...
     * @return new Pca9685
     */
    public static PCA9685_test create(String bus) {
        Log.i("PCA","Create 1");
        int fullAddress = BASE_ADDRESS;
        return new PCA9685_test(getDevice(bus, fullAddress));
    }
    ...
    public void setup() throws InterruptedException, IOException {
        ...
        device.writeRegBuffer(ALL_LED_ON_L, buffer, buffer.length);

        device.writeRegByte(MODE_2_ADDR, (byte) OUTDRV);
        device.writeRegByte(MODE_1_ADDR, (byte) ALLCALL);
        Thread.sleep(5);

        byte mode1 = device.readRegByte(MODE_1_ADDR);
        mode1 = (byte) (mode1 & ~SLEEP);
        device.writeRegByte(MODE_1_ADDR, mode1);
        Thread.sleep(5);
    }
